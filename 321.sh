...
# [BUG-2 修复] LOCK_FILE 路径更新为 flock 使用的标准位置
LOCK_FILE="/var/lock/personal_backup_rclone.lock"
...
save_config() {
    ...
    # [BUG-1 修复] 使用更安全的分隔符
    BACKUP_SOURCE_PATHS_STRING=$(IFS="$SAFE_SEPARATOR"; echo "${BACKUP_SOURCE_PATHS_ARRAY[*]}")
    RCLONE_TARGETS_STRING=$(IFS="$SAFE_SEPARATOR"; echo "${RCLONE_TARGETS_ARRAY[*]}")
    ENABLED_RCLONE_TARGET_INDICES_STRING=$(IFS="$SAFE_SEPARATOR"; echo "${ENABLED_RCLONE_TARGET_INDICES_ARRAY[*]}")
    RCLONE_TARGETS_METADATA_STRING=$(IFS="$SAFE_SEPARATOR"; echo "${RCLONE_TARGETS_METADATA_ARRAY[*]}")
...
load_config() {
    if [[ -f "$CONFIG_FILE" ]]; then
        ...
        if [[ -n "$BACKUP_SOURCE_PATHS_STRING" ]]; then
            read -r -a BACKUP_SOURCE_PATHS_ARRAY <<< "$BACKUP_SOURCE_PATHS_STRING"
        else
            BACKUP_SOURCE_PATHS_ARRAY=()
        fi
        if [[ -n "$RCLONE_TARGETS_STRING" ]]; then
            read -r -a RCLONE_TARGETS_ARRAY <<< "$RCLONE_TARGETS_STRING"
        else
            RCLONE_TARGETS_ARRAY=()
        fi
        if [[ -n "$ENABLED_RCLONE_TARGET_INDICES_STRING" ]]; then
            read -r -a ENABLED_RCLONE_TARGET_INDICES_ARRAY <<< "$ENABLED_RCLONE_TARGET_INDICES_STRING"
        else
            ENABLED_RCLONE_TARGET_INDICES_ARRAY=()
        fi
        if [[ -n "$RCLONE_TARGETS_METADATA_STRING" ]]; then
            read -r -a RCLONE_TARGETS_METADATA_ARRAY <<< "$RCLONE_TARGETS_METADATA_STRING"
        fi
...
# [BUG-2 修复] 将核心逻辑封装，以便被 flock 调用
run_main_logic() {
    initialize_directories

    TEMP_DIR=$(mktemp -d -t personal_backup_rclone_XXXXXX)
    if [ ! -d "$TEMP_DIR" ]; then
        echo -e "${RED}[ERROR] 无法创建临时目录。${NC}" >&2
        exit 1
    fi
...
main() {
    ...
    (
        flock -n 200 || { 
            # 在日志系统初始化前，只能 echo 到 stderr
            echo -e "${RED}另一个脚本实例正在运行。已退出。${NC}" >&2
            exit 1
        }
        # flock 成功获取锁，执行主逻辑
        run_main_logic "$@"
