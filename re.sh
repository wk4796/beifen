...
cleanup() {
    ...
    if [ -f "$LOCK_FILE" ] && [ "$(cat "$LOCK_FILE")" -eq "$$" ]; then
        rm -f "$LOCK_FILE"
        echo "$(date '+%Y-%m-%d %H:%M:%S') - 移除进程锁: $LOCK_FILE" >> "$LOG_FILE"
    fi
}
...
acquire_lock() {
    if [ -e "$LOCK_FILE" ]; then
        ...
        if ps -p "$old_pid" > /dev/null; then
            log_and_display "${RED}错误：另一个脚本实例 (PID: $old_pid) 正在运行。已退出。${NC}"
            exit 1
        else
            log_and_display "${YELLOW}警告：发现一个过期的锁文件 (PID: $old_pid)，已自动移除。${NC}"
            rm -f "$LOCK_FILE"
        fi
    fi
    echo $$ > "$LOCK_FILE"
...
rotate_log_if_needed() {
    ...
    if (( log_size > LOG_MAX_SIZE_BYTES )); then
        ...
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Log file size > 8MB. Rotating. Old log is in ${rotated_log_file}" >> "$LOG_FILE"
        mv "$LOG_FILE" "${rotated_log_file}"
        touch "$LOG_FILE"
        log_and_display "${YELLOW}日志文件已轮转 (超过 8MB)，旧日志已保存为 ${rotated_log_file}。${NC}"
    fi
}
    ...
    if [ ${#missing_deps[@]} -gt 0 ]; then
        ...
        if [[ "$choice" =~ ^[Yy]$ ]]; then
            ...
            if command -v apt-get &> /dev/null && [ ${#apt_packages[@]} -gt 0 ]; then
                log_and_display "正在使用 apt-get 安装: ${apt_packages[*]}..." "${YELLOW}"
                if sudo apt-get update && sudo apt-get install -y "${apt_packages[@]}"; then
                    log_and_display "${GREEN}软件包安装成功。${NC}"
                    installed_something=1
                else
                    log_and_display "${RED}软件包安装失败。${NC}"
                fi
            fi

            if [[ " ${missing_deps[*]} " =~ " rclone " ]]; then
                log_and_display "正在安装/更新 Rclone..." "${YELLOW}"
                if curl https://rclone.org/install.sh | sudo bash; then
                    log_and_display "${GREEN}Rclone 安装成功。${NC}"
                    installed_something=1
                else
                    log_and_display "${RED}Rclone 安装失败。${NC}"
                fi
            fi
...
restore_backup() {
    ...
    log_and_display "发现以下备份文件（按名称逆序排序）："
    ...
    if [[ -n "$RCLONE_BWLIMIT" ]]; then
        ...
        log_and_display "下载将使用带宽限制: ${RCLONE_BWLIMIT}" "${BLUE}"
    fi
...
manual_backup() {
    ...
    if [ ${#BACKUP_SOURCE_PATHS_ARRAY[@]} -eq 0 ]; then
        ...
        log_and_display "${YELLOW}请先在选项 [3] 中添加要备份的路径。${NC}"
        press_enter_to_continue
        return 1
    fi
    if [ ${#ENABLED_RCLONE_TARGET_INDICES_ARRAY[@]} -eq 0 ]; then
        log_and_display "${RED}错误：没有启用任何 Rclone 备份目标。${NC}"
        log_and_display "${YELLOW}请先在选项 [5] 中配置并启用一个或多个目标。${NC}"
        press_enter_to_continue
        return 1
    fi
...
add_backup_path() {
    ...
    if [[ -z "$path_input" ]]; then
        log_and_display "${RED}错误：路径不能为空。${NC}"
        press_enter_to_continue
        return
    fi
    ...
    if [[ -z "$resolved_path" ]]; then
        log_and_display "${RED}错误：输入的路径 '$path_input' 无效或不存在。${NC}"
...
set_packaging_strategy() {
    ...
    case $choice in
        1)
            ...
            save_config
            log_and_display "${GREEN}打包策略已设置为: 每个源单独打包。${NC}"
            ;;
        2)
            PACKAGING_STRATEGY="single"
            save_config
            log_and_display "${GREEN}打包策略已设置为: 所有源打包成一个。${NC}"
            ;;
        *)
            log_and_display "${RED}无效选项。${NC}"
            ;;
...
set_backup_mode() {
    ...
    case $choice in
        1)
            BACKUP_MODE="archive"
            save_config
            log_and_display "${GREEN}备份模式已设置为: 归档模式。${NC}"
            ;;
        2)
            BACKUP_MODE="sync"
            save_config
            log_and_display "${GREEN}备份模式已设置为: 同步模式。${NC}"
            ;;
        *)
            log_and_display "${RED}无效选项。${NC}"
            ;;
...
toggle_integrity_check() {
    ...
    if [[ "$choice" =~ ^[Yy]$ ]]; then
        if [[ "$ENABLE_INTEGRITY_CHECK" == "true" ]]; then
            ...
            log_and_display "${YELLOW}完整性校验已关闭。${NC}"
        else
            ENABLE_INTEGRITY_CHECK="true"
            log_and_display "${GREEN}完整性校验已开启。${NC}"
        fi
        save_config
    else
        log_and_display "状态未改变。"
    fi
    press_enter_to_continue
}
        ...
        if [[ "$RETENTION_POLICY_TYPE" == "count" ]]; then
            ...
            if [ "$num_to_delete" -gt 0 ]; then
                ...
                for (( i=0; i<num_to_delete; i++ )); do
                    ...
                    log_and_display "正在删除: ${target_path_for_delete}${file_to_delete}"
...
check_temp_space() {
    ...
    log_and_display "正在计算所需临时空间..." "${BLUE}"
    ...
    log_and_display "预估需要临时空间: ~${required_hr}, 可用空间: ${available_hr}"
...
perform_sync_backup() {
    ...
    log_and_display "${BLUE}--- ${backup_type} 过程开始 (同步模式) ---${NC}"
    send_telegram_message "*${SCRIPT_NAME}：开始 (${backup_type} - 同步模式)*\n时间: ${readable_time}\n将同步 ${total_paths_to_backup} 个路径。"
    log_and_display "备份模式: [同步模式]。${YELLOW}保留策略和恢复功能在此模式下不可用。${NC}"

    local bw_limit_arg=""
    if [[ -n "$RCLONE_BWLIMIT" ]]; then
        bw_limit_arg="--bwlimit ${RCLONE_BWLIMIT}"
        log_and_display "同步将使用带宽限制: ${RCLONE_BWLIMIT}" "${BLUE}"
    fi
    ...
    for ((i=0; i<total_paths_to_backup; i++)); do
        ...
        log_and_display "${BLUE}--- 正在处理路径 $((i+1))/${total_paths_to_backup}: ${path_to_sync} ---${NC}"
        
        if [[ ! -e "$path_to_sync" ]]; then
            log_and_display "${RED}错误：路径 '$path_to_sync' 不存在，跳过。${NC}"
        ...
        for enabled_idx in "${ENABLED_RCLONE_TARGET_INDICES_ARRAY[@]}"; do
            ...
            log_and_display "正在同步 ${path_to_sync} 到 ${sync_destination}..."
            if rclone sync "$path_to_sync" "$sync_destination" --progress ${bw_limit_arg}; then
                log_and_display "${GREEN}同步到 ${rclone_target} 成功！${NC}"
                path_sync_succeeded="true"
                any_sync_succeeded="true"
                sync_statuses+="\n- \`${rclone_target}\`: 同步成功"
            else
                log_and_display "${RED}同步到 ${rclone_target} 失败！${NC}"
                sync_statuses+="\n- \`${rclone_target}\`: 同步失败"
            fi
        done
    ...
    log_and_display "${BLUE}--- ${backup_type} (同步模式) 过程结束 ---${NC}"
...
perform_archive_backup() {
    ...
    log_and_display "${BLUE}--- ${backup_type} 过程开始 (归档模式) ---${NC}"
    ...
    if [[ "$PACKAGING_STRATEGY" == "single" ]]; then
        ...
        log_and_display "打包策略: [每个源单独打包]。" "${BLUE}"
        for i in "${!BACKUP_SOURCE_PATHS_ARRAY[@]}"; do
    ...
    log_and_display "${BLUE}--- ${backup_type} (归档模式) 过程结束 ---${NC}"
...
upload_archive() {
    ...
    if [[ -n "$RCLONE_BWLIMIT" ]]; then
        bw_limit_arg="--bwlimit ${RCLONE_BWLIMIT}"
        log_and_display "上传将使用带宽限制: ${RCLONE_BWLIMIT}" "${BLUE}"
    fi
    ...
    for enabled_idx in "${ENABLED_RCLONE_TARGET_INDICES_ARRAY[@]}"; do
        ...
        if rclone copyto "$temp_archive_path" "${destination_path}${archive_name}" --progress ${bw_limit_arg}; then
            log_and_display "上传到 ${rclone_target} 成功！" "${GREEN}"
            upload_statuses+="\n- \`${rclone_target}\`: 上传成功"
            any_upload_succeeded_for_path="true"

            if [[ "$ENABLE_INTEGRITY_CHECK" == "true" ]]; then
                log_and_display "正在对 ${rclone_target} 上的文件进行完整性校验..." "${BLUE}"
                if rclone check "$temp_archive_path" "${destination_path}${archive_name}"; then
                    log_and_display "校验成功！文件完整。" "${GREEN}"
                    upload_statuses+=" (校验通过 ✔️)"
                else
                    log_and_display "校验失败！云端文件可能已损坏！" "${RED}"
                    upload_statuses+=" (**校验失败 ❌**)"
                fi
            fi
        else
            log_and_display "上传到 ${rclone_target} 失败！" "${RED}"
            upload_statuses+="\n- \`${rclone_target}\`: 失败"
        fi
    done
...
system_maintenance_menu() {
    while true; do
        display_header
        echo -e "${BLUE}=== 11. 系统维护 ===${NC}"
        echo ""
        local log_info="(文件不存在)"
        if [[ -f "$LOG_FILE" ]]; then
            local log_size
            log_size=$(du -h "$LOG_FILE" 2>/dev/null | awk '{print $1}')
            log_info="(大小: ${log_size}, 路径: ${LOG_FILE})"
        fi
        echo -e "  1. ${YELLOW}查看日志文件${NC} ${log_info}"
        echo ""
        echo -e "  0. ${RED}返回主菜单${NC}"
        read -rp "请输入选项: " choice
        ...
        case $choice in
            1)
                if [[ -f "$LOG_FILE" ]]; then
                    ...
                    log_and_display "${YELLOW}日志文件不存在。${NC}"
                    press_enter_to_continue
                fi
                ;;
            0) break ;;
...
uninstall_script() {
    ...
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        ...
        rm -f "${LOG_FILE}".*.rotated 2>/dev/null && log_and_display "删除轮转日志"
